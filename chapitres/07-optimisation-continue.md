...
Imaginez une randonneuse perdue dans le brouillard sur une montagne. Elle ne voit rien autour d'elle, mais sous ses pieds elle sent la pente. Pour descendre dans la vallée le plus vite possible, elle fait un pas dans la direction où le sol descend le plus fort, puis recommence. Voilà, en une phrase, toute l'idée de la descente de gradient (gradient descent). C'est l'algorithme (un **algorithme** est une recette précise, une suite d'étapes à suivre mécaniquement pour obtenir un résultat, comme les instructions d'un manuel de montage) qui fait tourner aujourd'hui la quasi-totalité de l'apprentissage automatique (machine learning : la branche de l'informatique où l'on fait *apprendre* une tâche à une machine à partir d'exemples, plutôt que de lui dicter toutes les règles), du plus petit modèle de régression (une **régression** cherche à prédire un nombre à partir d'autres nombres, par exemple deviner le prix d'un appartement d'après sa surface) au plus gros réseau de neurones (un **réseau de neurones** est un très grand modèle fait de nombreuses petites unités de calcul connectées, vaguement inspiré des neurones du cerveau).
...
On se donne une fonction (une **fonction**, c'est une machine qui prend un nombre, ou une liste de nombres, en entrée et ressort un résultat, comme une recette qui transforme des ingrédients en plat) $`f: \mathbb{R}^n \to \mathbb{R}`$ que l'on appelle **fonction objectif** (objective function), ou **fonction de coût** (cost / loss function). Elle prend en entrée un vecteur de paramètres et renvoie un seul nombre : « à quel point c'est mauvais ». Notre but est de trouver le vecteur qui rend ce nombre le plus petit possible.
...
On suppose $`f`$ différentiable (c'est-à-dire **lisse, sans cassure ni angle pointu** : en chaque point la courbe a une pente bien définie, comme une route sans marche d'escalier où l'on peut poser une règle qui touche la courbe : cette pente s'appelle la **dérivée**, le « taux de variation », qui dit de combien la sortie monte ou descend quand on pousse l'entrée d'un poil). Son **gradient** au point $`x`$, noté $`\nabla f(x)`$, est le vecteur de toutes ses **dérivées partielles** (une dérivée partielle, c'est la pente quand on ne bouge qu'**une seule** des entrées en gardant toutes les autres figées, comme régler un seul bouton d'une console en laissant les autres tranquilles). On l'a vu aux chapitres précédents, on le réutilise simplement ici.
...
Le résultat fondamental qui justifie l'algorithme est le suivant. Faisons un petit pas $`d`$ (un vecteur de déplacement) depuis $`x`$. Le développement de Taylor au premier ordre (une **approximation** : pour un tout petit pas, on remplace la vraie courbe, compliquée, par sa droite tangente, bien plus simple, comme on assimile un bout de trottoir courbe à un segment droit quand on regarde de très près) donne :
...
Pour faire *baisser* $`f`$ le plus vite possible avec un pas de longueur fixée, on doit choisir la direction $`d`$ qui rend $`\nabla f(x)^\top d`$ le plus négatif possible. Or, par l'inégalité de Cauchy–Schwarz (une règle classique qui dit que le produit scalaire de deux flèches ne peut jamais dépasser, en valeur, le produit de leurs longueurs : il est le plus grand quand elles pointent dans le même sens, et le plus négatif quand elles pointent en sens opposés), $`\nabla f(x)^\top d \ge -\|\nabla f(x)\|\,\|d\|`$, avec égalité exactement quand $`d`$ pointe à l'opposé du gradient. **La direction de plus forte descente est donc $`d = -\nabla f(x)`$.** C'est là tout le secret : on avance dans le sens inverse de la pente.
...
Prenons la fonction la plus simple qui soit instructive : $`f(x) = x^2`$, en dimension 1 (« dimension 1 » veut dire qu'il n'y a **qu'un seul réglage** à régler, un seul nombre $`x`$ ; la dimension, c'est le nombre de cases de la fiche, donc ici une seule case). On sait que le minimum est en $`x^\star = 0`$. Le gradient (ici la dérivée) vaut $`f'(x) = 2x`$ (le petit trait $`'`$ après $`f`$ se lit **« prime »** : $`f'`$ se dit « f prime » et désigne la dérivée de $`f`$, c'est-à-dire sa pente ; quand il n'y a qu'une seule entrée, gradient et dérivée sont la même chose). La mise à jour devient (le point centré $`\cdot`$, comme dans $`\eta \cdot 2x_k`$, est un simple **signe de multiplication**, exactement comme le $`\times`$ de l'école : il se lit « fois ») :
...
À partir du lagrangien, on définit la **fonction duale** $`\,q(\lambda, \mu) = \min_x \mathcal{L}(x, \lambda, \mu)`$ (la **fonction duale** : pour des prix $`\lambda, \mu`$ donnés, on rend le lagrangien aussi petit que possible en jouant sur $`x`$, et le résultat est une fonction des seuls prix). Pour tout point réalisable $`x`$ (donc $`g_i(x)=0`$ et $`h_j(x)\le 0`$) et tout $`\mu \ge 0`$, on a $`\mathcal{L}(x,\lambda,\mu) = f(x) + 0 + \sum_j \mu_j h_j(x) \le f(x)`$, car chaque terme $`\mu_j h_j(x)`$ est $`\le 0`$. En minimisant le membre de gauche sur *tout* $`x`$, on obtient une **borne inférieure** (un nombre dont on est **sûr** qu'il est plus petit ou égal à ce qu'on cherche, un « plancher » garanti) sur la valeur optimale $`p^\star`$ du problème de départ. En effet, chercher le plus petit sur *tous* les $`x`$ (même ceux qui violent les contraintes) ne peut donner qu'un résultat encore plus petit que de chercher le plus petit sur les seuls $`x`$ autorisés : élargir le terrain de recherche ne peut que faire baisser le minimum. Donc $`q(\lambda,\mu) = \min_{\text{tous } x} \mathcal{L} \le \min_{x \text{ réalisable}} \mathcal{L} \le \min_{x \text{ réalisable}} f = p^\star`$ : c'est la **dualité faible** $`q(\lambda,\mu) \le p^\star`$. Le **problème dual** consiste à remonter cette borne le plus haut possible :
...
En réécrivant chaque contrainte sous la forme standard $`h_i(\theta,b) = 1 - y_i(\theta^\top x_i + b) \le 0`$, en formant le lagrangien et en appliquant les KKT, la complémentarité fait tout le travail : seuls les points **sur la marge** ($`y_i(\theta^\top x_i + b) = 1`$) ont un multiplicateur $`\alpha_i > 0`$ (la lettre grecque $`\alpha`$ se lit **« alpha »** ; c'est ici le nom donné au multiplicateur de chaque exemple). Ce sont les **vecteurs de support** (support vectors), les seuls qui déterminent la frontière ; tous les autres exemples pourraient disparaître sans rien changer. Le problème dual ne dépend des données qu'à travers des produits scalaires $`x_i^\top x_j`$, ce qui ouvre la porte à l'**astuce du noyau** (kernel trick : un **noyau** est une fonction qui calcule directement le produit scalaire « comme si » les données avaient été projetées dans un espace bien plus grand, sans jamais faire le voyage ; cela permet de séparer par une frontière courbe des données qu'aucune ligne droite ne pourrait trancher) pour séparer des données non linéairement séparables.
