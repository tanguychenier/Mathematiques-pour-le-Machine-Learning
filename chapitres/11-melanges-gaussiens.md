...
![Un melange de gaussiennes : une forme compliquee est la somme de plusieurs cloches simples (les groupes)](../assets/melange-gaussiennes.svg)
...
> **Que veut dire « paramètres » ?** Ce sont les nombres de réglage du modèle : ici la position, la largeur et l'importance de chaque cloche, comme les boutons d'une radio qu'on tourne pour bien capter.
...
> **Que veut dire « contrainte » ?** C'est une condition obligatoire que la solution doit respecter, comme une règle du jeu : ici, les parts doivent toutes être positives et faire un tout.
...
> **Que veut dire « modèle génératif » ?** C'est un modèle capable non seulement de reconnaître, mais de **fabriquer de nouvelles données réalistes** lui-même, comme un programme qui invente des visages ou des phrases plausibles. Le mot *génératif* vient de « générer », c'est-à-dire « produire ».
...
> **Piège central (le log d'une somme).** Pour une seule gaussienne, le $`\ln`$ « mange » l'exponentielle et tout se simplifie. Ici, le $`\ln`$ porte sur une **somme** $`\sum_k \pi_k \mathcal{N}(\cdot)`$: impossible de la casser proprement, car $`\ln(a+b) \ne \ln a + \ln b`$. C'est cette somme **à l'intérieur** du logarithme qui rend le problème difficile et interdit une **solution en forme close** ; quand il n'y en a pas, on doit procéder par essais successifs avec un **algorithme**. Tout l'algorithme EM (section suivante) est né pour contourner cet obstacle.

> **Que veut dire « solution en forme close » ?** Aussi appelée *solution close*, c'est une réponse qu'on peut écrire d'un coup avec une formule finie, comme « la moyenne = somme divisée par le nombre ».

> **Que veut dire « algorithme » ?** C'est une recette précise, une suite d'étapes mécaniques qu'on exécute dans l'ordre pour obtenir un résultat, comme une recette de cuisine ou un mode d'emploi de meuble à monter.
...
> **Que veut dire « inférence variationnelle » ?** C'est une grande famille de méthodes qui, faute de pouvoir calculer la vérité a posteriori exacte, en cherchent la **meilleure approximation** au sein d'une famille de lois choisie. Le mot « variationnelle » vient de ce qu'on fait *varier* cette loi-brouillon jusqu'à la meilleure.
