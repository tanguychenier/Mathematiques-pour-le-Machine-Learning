...
> **Le symbole $`\ker`$ (noyau, kernel).** $`\ker(M)`$ représente le **noyau** de la matrice $`M`$: l'ensemble de tous les vecteurs que $`M`$ envoie sur zéro. Imaginez une machine qui « avale » certains vecteurs et les réduit à néant : le noyau est l'ensemble de ces vecteurs avalés.
...
> **Le symbole $`x^\top A x`$ (forme quadratique).** Cette expression représente un **nombre** obtenu en « sandwichant » la matrice entre un vecteur ligne $`x^\top`$ (à gauche) et le même vecteur colonne $`x`$ (à droite). Imaginez que vous mesurez une « énergie » associée à la direction $`x`$. Si cette énergie est toujours strictement positive (sauf en zéro), la matrice est définie positive : c'est une cuvette (bol) parfaitement convexe, qui a un unique point le plus bas.
